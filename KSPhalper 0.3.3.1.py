...
class KSPHelper:
    ...
    def get_text(self, key):
        texts = {
            "russian": {
                ...
                "hotkey": "⌨️ Горячие клавиши",
                "hotkey_desc": "Аварийное закрытие:",
                "hotkey_warning": "⚠️ Сочетание клавиш для немедленного закрытия программы"
            },
            "english": {
                ...
                "hotkey": "⌨️ Hotkeys",
                "hotkey_desc": "Emergency exit:",
                "hotkey_warning": "⚠️ Keyboard shortcut to close the program"
            },
            "german": {
                ...
                "hotkey": "⌨️ Tastenkürzel",
                "hotkey_desc": "Notausstieg:",
                "hotkey_warning": "⚠️ Tastenkombination zum sofortigen Schließen"
            },
            "portuguese": {
                ...
                "hotkey": "⌨️ Teclas de atalho",
                "hotkey_desc": "Saída de emergência:",
                "hotkey_warning": "⚠️ Atalho para fechar o programa"
            },
            "spanish": {
                ...
                "total_dv_text": "ΔV Total",
                "hotkey": "⌨️ Teclas rápidas",
                "hotkey_desc": "Salida de emergencia:",
                "hotkey_warning": "⚠️ Atajo para cerrar el programa"
            },
            "french": {
                ...
                "total_dv_text": "ΔV Total",
                "hotkey": "⌨️ Raccourcis clavier",
                "hotkey_desc": "Fermeture d'urgence:",
                "hotkey_warning": "⚠️ Raccourci pour fermer le programme"
            },
            "italian": {
                ...
                "hotkey": "⌨️ Tasti rapidi",
                "hotkey_desc": "Uscita di emergenza:",
                "hotkey_warning": "⚠️ Scorciatoia per chiudere il programma"
            }
        }
    ...
    def update_widget_text(self, widget):
        if isinstance(widget, ttk.Label):
            ...
            elif "Горячие клавиши" in current or "Hotkeys" in current:
                widget.config(text=self.get_text("hotkey"))
            elif "Аварийное закрытие" in current or "Emergency exit" in current:
                widget.config(text=self.get_text("hotkey_desc"))
    ...
    def init_settings_tab(self):
        ...
        hotkey_frame = ttk.LabelFrame(settings_frame, text=self.get_text("hotkey"), padding="15")
        hotkey_frame.pack(fill='x', pady=(0, 15))
        
        ttk.Label(hotkey_frame, text=self.get_text("hotkey_desc")).pack(anchor='w')
        ...
        ttk.Label(hotkey_frame, 
                  text=self.get_text("hotkey_warning"),
                  font=('Arial', 8, 'italic'),
    ...
    def apply_hotkey(self):
        ...
        old_keys = ["Control-q", "Control-x", "Control-w", "Alt-F4", "Control-Shift-q", "Control-Alt-x"]
        for key in old_keys:
            try:
                self.root.unbind_all(key)
            except:
                pass
        tk_key = self.convert_to_tkinter(hotkey)
        self.root.bind_all(tk_key, self.emergency_exit)
    
    def convert_to_tkinter(self, hotkey):
        mapping = {
            "Ctrl": "Control",
            "Alt": "Alt",
            "Shift": "Shift",
        ...
        result = []
        for part in parts:
            result.append(mapping.get(part, part.lower()))
        return '<' + '-'.join(result) + '>'
    
    def emergency_exit(self, event=None):
        if messagebox.askyesno("⚠️ Аварийное закрытие", 
    ...
    def init_delta_calculator(self):
        ...
        # ВСЕ 15 ПЛАНЕТ!
        self.gravity = ttk.Combobox(params_frame, values=[
            "9.81 (Kerbin)",
            "1.63 (Mun)", 
            "3.71 (Duna)",
            "0.16 (Gilly)",
    ...
    def get_orbit_advice(self, dv, body):
        ...
        if body in requirements:
            req = requirements[body]
            if dv >= req:
                return f"{self.get_text('enough')} {body}\n{self.get_text('stock')} {dv - req:.0f} м/с"
            else:
                return f"{self.get_text('not_enough')} {body}\n{self.get_text('missing')} {req - dv:.0f} м/с"
        return ""
    ...
    def show_rocket_advice(self):
        ...
        advice += f"📊 Рекомендации для {size_names.get(size, size)} ракеты:\n\n"
